...
function chooseBestPickForAgent(draftObj, playerIdx, simsPerCandidate = 6) {
  ...
  for (const c of candidates) results.set(c.name + '|' + c.pickType, 0);
  ...
  for (const choice of candidates) {
    ...
    for (let s = 0; s < simsPerCandidate; s++) {
      ...
      results.set(key, results.get(key) + finalScore);
    }
  }
  ...
  for (const c of candidates) {
    const key = c.name + '|' + c.pickType;
    const sum = results.get(key) || 0;
    const avg = sum / simsPerCandidate;
...
function initSetupUI() {
  ...
  // Note: simulation slider removed — Deep uses a single rollout per candidate now.
  // Button groups
