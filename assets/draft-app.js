...
const el = {
  ...
  bestPickAlgoGroup: document.getElementById("bestPickAlgoGroup"),
  randomLore: document.getElementById("randomLore"),
  simulationControls: document.getElementById("simulationControls"),
  bestPickControls: document.getElementById("bestPickControls"),
...
function cloneDraft(src) {
  ...
  // restore function-less card objects (they are plain data already)
  return copy;
}

function getPickableCardsForDraft(playerIdx, draftObj) {
  const player = draftObj.players[playerIdx];
  const cards = [];
  if (!player.leader) {
    for (const c of draftObj.availableLeaders) cards.push({ ...c, pickType: 'leader' });
  }
  if (player.lore.length < getLoreCountForPlayer(playerIdx, draftObj)) {
    for (const c of draftObj.availableLore) cards.push({ ...c, pickType: 'lore' });
  }
  return cards;
}

function getWeightedScoreForDraft(entry, type, draftObj) {
  const w = type === 'leader' ? draftObj.leaderWeight : draftObj.loreWeight;
  return (entry.value || 0) * w;
}
// Synergy table loaded from CSV: Map<normalizedRowKey, Map<normalizedColKey, number>>
...
function parseBonusCell(cell) {
  ...
  // Parse per-player mappings like "3p: 4; 4p: 2" or "3p=4,4p=2"
  const parts = s.split(/[;,]/).map(p => p.trim()).filter(Boolean);
  const mapping = {};
  for (const part of parts) {
    const m = part.match(/^(\d+)p\s*[:=]\s*([+-]?\d+(?:\.\d+)?)$/i);
    if (m) {
      mapping[Number(m[1])] = Number(m[2]);
    }
  }
  if (Object.keys(mapping).length > 0) {
    // Return a function that selects value by numPlayers, with rule: 2p uses 3p if unspecified
    return function(numPlayers) {
      ...
      // fallback to any single mapping value if present
  ...
  // Could not parse, try to extract a number inside text
...
function getSynergyBonusForDraft(player, draftObj) {
  // Compute synergy bonuses. Prefer CSV table if available (supports tag lookups and functions).
  let bonus = 0;
  if (SYNERGY_TABLE) {
    // leader -> lore
    if (player.leader) {
      const rowKeys = [normalizeText(player.leader.name)];
      if (player.leader.card && Array.isArray(player.leader.card.tags)) {
        for (const t of player.leader.card.tags) rowKeys.push(normalizeText(t));
      }

      for (const l of player.lore) {
        const colKeys = [normalizeText(l.name)];
        if (l.card && Array.isArray(l.card.tags)) {
    ...
    // lore <-> lore pairs (unordered)
    for (let i = 0; i < player.lore.length; i++) {
      for (let j = i + 1; j < player.lore.length; j++) {
        const a = player.lore[i];
        const b = player.lore[j];
        const aRow = [normalizeText(a.name)];
        const bCol = [normalizeText(b.name)];
  ...
  // Fallback to legacy SYNERGIES array (string-based entries). Check leader->lore and lore->lore.
  ...
  // lore-lore fallback: if player has both names referenced in SYNERGIES, add bonus
  for (const synergy of SYNERGIES) {
...
function playerTotalScoreForDraft(player, playerIndex, draftObj) {
  let score = 0;
  if (player.leader) score += getWeightedScoreForDraft(player.leader, 'leader', draftObj);
  for (const l of player.lore) score += getWeightedScoreForDraft(l, 'lore', draftObj);
  score += getSynergyBonusForDraft(player, draftObj);
  const handicap = (draftObj.handicaps && draftObj.handicaps[playerIndex]) || 0;
  score += handicap * draftObj.leaderWeight;
  return score;
}

function isPlayerDoneForDraft(player, playerIndex, draftObj) {
  return player.leader !== null && player.lore.length >= getLoreCountForPlayer(playerIndex, draftObj);
}

function chooseGreedyPickForDraft(draftObj, playerIdx) {
  const player = draftObj.players[playerIdx];
  const cards = getPickableCardsForDraft(playerIdx, draftObj);
  if (cards.length === 0) return null;

  // Evaluate immediate value (weighted + immediate synergy)
  let best = null;
  let bestVal = -Infinity;
  for (const card of cards) {
    let val = getWeightedScoreForDraft(card, card.pickType === 'leader' ? 'leader' : 'lore', draftObj);
    if (card.pickType === 'leader' && !player.leader) {
    ...
    // Check lore-lore immediate synergies (player already has other lore)
    if (card.pickType === 'lore' && player.lore.length > 0) {
      // Table-based lookups
      if (SYNERGY_TABLE) {
        for (const existing of player.lore) {
          const rowKeys = [normalizeText(existing.name)];
          const colKeys = [normalizeText(card.name)];
    ...
    if (val > bestVal) { bestVal = val; best = card; }
  }
  return best;
}

// Opponent pick: evaluate all options by running small rollouts and choose
// the pick that maximizes the opponent's expected final score.
function chooseBestPickForAgent(draftObj, playerIdx, simsPerCandidate = 6) {
  const candidates = getPickableCardsForDraft(playerIdx, draftObj);
  if (candidates.length === 0) return null;
  // Collect per-candidate statistics: own expected score and others' combined expected score
  const stats = new Map();
  for (const c of candidates) stats.set(c.name + '|' + c.pickType, { ownSum: 0, othersSum: 0, count: 0 });

  for (const choice of candidates) {
    const key = choice.name + '|' + choice.pickType;
    for (let s = 0; s < simsPerCandidate; s++) {
      const simDraft = cloneDraft(draftObj);
      simDraft.players = simDraft.players.map(p => ({ leader: p.leader ? { ...p.leader } : null, lore: p.lore.map(x => ({ ...x })) }));
      simDraft.availableLeaders = simDraft.availableLeaders.map(x => ({ ...x }));
      simDraft.availableLore = simDraft.availableLore.map(x => ({ ...x }));
      simDraft.pickIndex = draftObj.pickIndex;

      const player = simDraft.players[playerIdx];
      if (choice.pickType === 'leader') {
        const idx = simDraft.availableLeaders.findIndex(x => x.name === choice.name);
        if (idx !== -1) simDraft.availableLeaders.splice(idx, 1);
        player.leader = { ...choice };
      } else {
        const idx = simDraft.availableLore.findIndex(x => x.name === choice.name);
        if (idx !== -1) simDraft.availableLore.splice(idx, 1);
        player.lore.push({ ...choice });
      }

      // simulate rest greedily (fast approximation for opponent evaluation)
      let safety = 10000;
      while (safety-- > 0) {
        simDraft.pickIndex++;
        const drafter = simDraft.order[simDraft.pickIndex % simDraft.order.length];
        const pIdx = drafter - 1;
        if (isPlayerDoneForDraft(simDraft.players[pIdx], pIdx, simDraft)) {
          if (simDraft.players.every((pl, i) => isPlayerDoneForDraft(pl, i, simDraft))) break;
          continue;
        }
        const pick = chooseGreedyPickForDraft(simDraft, pIdx);
        if (!pick) break;
        if (pick.pickType === 'leader') {
          const idx2 = simDraft.availableLeaders.findIndex(x => x.name === pick.name);
          if (idx2 !== -1) simDraft.availableLeaders.splice(idx2, 1);
          simDraft.players[pIdx].leader = { ...pick };
        } else {
          const idx2 = simDraft.availableLore.findIndex(x => x.name === pick.name);
          if (idx2 !== -1) simDraft.availableLore.splice(idx2, 1);
          simDraft.players[pIdx].lore.push({ ...pick });
        }
        if (simDraft.players.every((pl, i) => isPlayerDoneForDraft(pl, i, simDraft))) break;
      }

      // compute final scores: own and others combined
      const ownFinal = playerTotalScoreForDraft(simDraft.players[playerIdx], playerIdx, simDraft);
      let othersFinal = 0;
      for (let i = 0; i < simDraft.players.length; i++) if (i !== playerIdx) othersFinal += playerTotalScoreForDraft(simDraft.players[i], i, simDraft);

      const st = stats.get(key);
      st.ownSum += ownFinal;
      st.othersSum += othersFinal;
      st.count += 1;
      stats.set(key, st);
    }
  }

  // Determine baseline (max own) and consider blocking choices per user rule:
  // Block if opponent's own loss (vs best-own choice) is smaller than the
  // reduction in others' combined score divided by number of players.
  let bestOwnKey = null; let bestOwnAvg = -Infinity;
  for (const [key, st] of stats.entries()) {
    const avgOwn = st.ownSum / st.count;
    if (avgOwn > bestOwnAvg) { bestOwnAvg = avgOwn; bestOwnKey = key; }
  }
  const bestOwnStats = stats.get(bestOwnKey);
  const bestOwnOthersAvg = bestOwnStats.othersSum / bestOwnStats.count;

  // Evaluate blocking candidates
  let chosenKey = bestOwnKey;
  let chosenDelta = 0;
  for (const [key, st] of stats.entries()) {
    if (key === bestOwnKey) continue;
    const avgOwn = st.ownSum / st.count;
    const avgOthers = st.othersSum / st.count;
    const ownLoss = bestOwnAvg - avgOwn;
    const othersLoss = bestOwnOthersAvg - avgOthers; // how much others lose if choose this key vs baseline
    const delta = (othersLoss / draftObj.numPlayers) - ownLoss;
  ...
  // Return the candidate object for chosenKey
  const [name, pickType] = chosenKey.split('|');
  return candidates.find(c => (c.name + '|' + c.pickType) === chosenKey) || null;
}

// ======= Simulation-based best pick =======
function getBestPickSim(playerIdx, sims = 30) {
  const candidates = getPickableCards(playerIdx);
  if (candidates.length === 0) return null;

  // For performance and determinism we run exactly one full rollout per candidate.
  // This applies the candidate, then simulates the rest of the draft once using
  // short opponent evaluations (1 inner sim) and returns the candidate with the
  // highest single-rollout final score.
  const basePickIndex = draft.pickIndex;
  let best = null;
  let bestScore = -Infinity;

  for (const choice of candidates) {
    const simDraft = cloneDraft(draft);
    simDraft.players = simDraft.players.map(p => ({ leader: p.leader ? { ...p.leader } : null, lore: p.lore.map(x => ({ ...x })) }));
    simDraft.availableLeaders = simDraft.availableLeaders.map(x => ({ ...x }));
    simDraft.availableLore = simDraft.availableLore.map(x => ({ ...x }));
    simDraft.pickIndex = basePickIndex;

    // Apply candidate pick for our player
    const player = simDraft.players[playerIdx];
    if (choice.pickType === 'leader') {
      const idx = simDraft.availableLeaders.findIndex(x => x.name === choice.name);
      if (idx !== -1) simDraft.availableLeaders.splice(idx, 1);
    ...
    // Clear lastSimScores for this run and record per-candidate heuristic
    draft.lastSimScores = draft.lastSimScores || {};
    // Simulate rest of draft where opponents try to maximize THEIR score.
    const simTrace = [];
    let safety = 10000;
    while (safety-- > 0) {
      simDraft.pickIndex++;
      ...
      // Use a single inner sim when evaluating opponent choices to keep cost low.
      const pick = chooseBestPickForAgent(simDraft, pIdx, 1);
      if (!pick) break;
      if (pick.pickType === 'leader') {
        const idx2 = simDraft.availableLeaders.findIndex(x => x.name === pick.name);
        if (idx2 !== -1) simDraft.availableLeaders.splice(idx2, 1);
      ...
      simTrace.push({ drafter: pIdx + 1, pick: `${pick.pickType}:${pick.name}` });
      if (simDraft.players.every((pl, i) => isPlayerDoneForDraft(pl, i, simDraft))) break;
    }

    const finalScore = playerTotalScoreForDraft(simDraft.players[playerIdx], playerIdx, simDraft);
    // store heuristic for UI display (overwrite per-candidate)
    try { draft.lastSimScores[choice.name + '|' + choice.pickType] = finalScore; } catch (e) {}
    if (draft.debugSim) {
      console.info('Sim candidate:', choice.pickType, choice.name, 'finalScore=', finalScore);
      console.info('Sim picks:', simTrace);
    }
    if (finalScore > bestScore) { bestScore = finalScore; best = choice; }
  }
...
function getSynergyBonus(player) {
  ...
  if (SYNERGY_TABLE) {
    ...
    if (player.leader) {
      ...
      for (const l of player.lore) {
        ...
        const val = lookupSynergyValue(rowKeys, colKeys, draft.numPlayers);
    ...
    for (let i = 0; i < player.lore.length; i++) {
      for (let j = i + 1; j < player.lore.length; j++) {
        ...
        if (a.card && Array.isArray(a.card.tags)) for (const t of a.card.tags) aRow.push(normalizeText(t));
        if (b.card && Array.isArray(b.card.tags)) for (const t of b.card.tags) bCol.push(normalizeText(t));
        const vab = lookupSynergyValue(aRow, bCol, draft.numPlayers);
        if (vab !== null) bonus += vab;
        const bRow = [normalizeText(b.name)];
        const aCol = [normalizeText(a.name)];
        if (b.card && Array.isArray(b.card.tags)) for (const t of b.card.tags) bRow.push(normalizeText(t));
        if (a.card && Array.isArray(a.card.tags)) for (const t of a.card.tags) aCol.push(normalizeText(t));
        const vba = lookupSynergyValue(bRow, aCol, draft.numPlayers);
  ...
  // Fallback to legacy SYNERGIES array
  for (const synergy of SYNERGIES) {
    if (player.leader && player.leader.name === synergy.leader) {
      const hasLore = player.lore.some(l => l.name === synergy.lore);
      if (hasLore) {
...
async function loadSynergySheet() {
  ...
  try {
    ...
    // First row contains column headers (first cell empty or label)
    const headerOriginal = rows[0].slice(1).map((h) => String(h ?? "").trim());
    const header = headerOriginal.map((h) => normalizeText(h));
    const table = new Map();
    const entries = [];
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      if (!row || row.length === 0) continue;
      const rowOriginal = String(row[0] ?? "").trim();
      const rowKey = normalizeText(rowOriginal);
      if (!rowKey) continue;
      const map = new Map();
      for (let j = 1; j < row.length && j - 1 < header.length; j++) {
        const colKey = header[j - 1];
        const colOriginal = headerOriginal[j - 1];
        const parsedVal = parseBonusCell(row[j]);
        // Ignore missing and zero values — only non-zero numeric synergies are stored
        // For functions, we keep them (they may return zero for some player counts)
        if (colKey && parsedVal !== null) {
          if (typeof parsedVal === 'number' && parsedVal === 0) continue;
          map.set(colKey, parsedVal);
          entries.push({ leader: rowOriginal, lore: colOriginal, bonus: parsedVal });
        }
      }
      if (map.size > 0) table.set(rowKey, map);
    }
    console.info("Loaded synergy CSV: rows=", rows.length, "headers=", header.length, table, entries);
...
function initSetupUI() {
  ...
  // Best pick algorithm selector
  if (el.bestPickAlgoGroup) {
    setupBtnGroup(el.bestPickAlgoGroup, (val) => {
      draft.bestPickAlgo = val;
      // Show or hide simulation controls in the top bar
      if (el.simulationControls) {
        el.simulationControls.style.display = val === 'simulation' ? 'flex' : 'none';
      }
    });
    // Initialize display according to current setting
    if (el.simulationControls) {
      el.simulationControls.style.display = draft.bestPickAlgo === 'simulation' ? 'flex' : 'none';
    }
  }
  // Simulation controls are intentionally empty (Deep uses single rollout per candidate)
  if (el.simulationControls) {
    el.simulationControls.innerHTML = '';
    el.simulationControls.style.display = draft.bestPickAlgo === 'simulation' ? 'flex' : 'none';
  }
  // Button groups
  ...
  // Pool highlight style toggle (pop vs brightness)
  if (el.poolHighlightGroup) {
    setupBtnGroup(el.poolHighlightGroup, (val) => { draft.poolHighlightStyle = val; clearPoolHighlights(); });
    // Ensure initial selection reflects state
    try {
      const sel = el.poolHighlightGroup.querySelector(`.btn-option[data-value="${draft.poolHighlightStyle}"]`);
      if (sel) {
        el.poolHighlightGroup.querySelectorAll('.btn-option').forEach(b => b.classList.remove('active'));
...
function clearPoolHighlights() {
  try {
    console.debug && console.debug('[highlight] clearPoolHighlights()');
    [el.leaderPool, el.lorePool].forEach(p => {
      if (!p) return;
      p.classList.remove('pool-dimmed','pool-highlight-mode-pop','pool-highlight-mode-brightness');
      p.querySelectorAll('.pool-card').forEach(c => {
        c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self','pool-card-hovered');
      });
    });
  } catch (e) {}
}

// Clear highlights across pools and available cards
function clearAllHighlights() {
  try {
    console.debug && console.debug('[highlight] clearAllHighlights()');
    // pools
    [el.leaderPool, el.lorePool].forEach(p => {
      if (!p) return;
      p.classList.remove('pool-dimmed','pool-highlight-mode-pop','pool-highlight-mode-brightness');
      p.querySelectorAll('.pool-card').forEach(c => {
        c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self');
      });
    });
    // available cards
    if (el.availableCards) {
      el.availableCards.classList.remove('pool-dimmed','pool-highlight-mode-pop','pool-highlight-mode-brightness');
      el.availableCards.querySelectorAll('.draft-card').forEach(c => {
        c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self','pool-card-hovered');
      });
    }
    // clear player-picked highlights
    if (el.playerBoards) {
      el.playerBoards.classList.remove('pool-dimmed','pool-highlight-mode-pop','pool-highlight-mode-brightness');
      el.playerBoards.querySelectorAll('.player-pick').forEach(c => {
        c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self','pool-card-hovered');
      });
    }
  } catch (e) {}
...
// Clear highlights in the available-draft area only
function clearDraftHighlights() {
  try {
    console.debug && console.debug('[highlight] clearDraftHighlights()');
    if (!el.availableCards) return;
    el.availableCards.classList.remove('pool-dimmed','pool-highlight-mode-pop','pool-highlight-mode-brightness');
    el.availableCards.querySelectorAll('.draft-card').forEach(c => {
      c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self','pool-card-hovered');
    });
    // also clear player-picked highlights
    if (el.playerBoards) {
      el.playerBoards.classList.remove('pool-dimmed','pool-highlight-mode-pop','pool-highlight-mode-brightness');
      el.playerBoards.querySelectorAll('.player-pick').forEach(c => {
        c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self','pool-card-hovered');
...
// Apply highlight for a hovered card name/type across pools and available cards
function applyHighlightsForCard(name, type, source = 'pool') {
  try {
    console.debug && console.debug('[highlight] applyHighlightsForCard()', {source, name, type, style: draft.poolHighlightStyle});
    if (draft.poolHighlightStyle === 'none') return;
    // compute partners map
    ...
    let hoveredSign = 0;
    for (const b of partners.values()) {
      if (b > 0) { hoveredSign = 1; break; }
      if (b < 0) hoveredSign = -1;
    }

    try { console.debug && console.debug('[highlight] partners:', Object.fromEntries(partners)); } catch(e) {}

    const mode = draft.poolHighlightStyle || 'pop';

    // Determine whether any partner cards are present in the DOM for this
    // hover source. If no partners will be highlighted anywhere in the
    // relevant containers, we should avoid highlighting the hovered card
    // itself (per user's preference).
    const presentNames = new Set();
    const addNamesFrom = (container, selector) => {
      if (!container) return;
      container.querySelectorAll(selector).forEach(el => {
        if (el.dataset && el.dataset.cardName) presentNames.add(el.dataset.cardName);
      });
    };
    if (source === 'pool') {
      addNamesFrom(el.leaderPool, '.pool-card');
      addNamesFrom(el.lorePool, '.pool-card');
      addNamesFrom(el.availableCards, '.draft-card');
      addNamesFrom(el.playerBoards, '.player-pick');
    } else if (source === 'draft') {
      addNamesFrom(el.availableCards, '.draft-card');
      addNamesFrom(el.playerBoards, '.player-pick');
    }
    let anyPartnerPresent = false;
    for (const pname of partners.keys()) {
      if (presentNames.has(pname)) { anyPartnerPresent = true; break; }
    }
    ...
    if (source === 'pool') {
      ...
      [el.leaderPool, el.lorePool].forEach(p => {
        if (!p) return;
        p.classList.remove('pool-highlight-mode-pop','pool-highlight-mode-brightness');
        p.classList.add(`pool-highlight-mode-${mode}`);
        p.querySelectorAll('.pool-card').forEach(c => {
          const other = c.dataset.cardName;
          if (!other) return;
          c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self','pool-card-hovered');
          if (other === name) {
            // always mark the hovered element as hovered so it won't be dimmed
            c.classList.add('pool-card-hovered');
            if (anyPartnerPresent) {
              if (hoveredSign > 0) c.classList.add('pool-card-highlight-positive');
              else if (hoveredSign < 0) c.classList.add('pool-card-highlight-negative');
              else c.classList.add('pool-card-highlight-self');
            }
          } else if (partners.has(other)) {
            const val = partners.get(other);
            if (val > 0) c.classList.add('pool-card-highlight-positive');
            else if (val < 0) c.classList.add('pool-card-highlight-negative');
          }
        });
        if (mode === 'brightness') p.classList.add('pool-dimmed');
      });
      // Also apply highlighting to available-draft cards, but only for cards
      // that are actually present in the available area (don't highlight
      // partner names that are not in the draft list).
      if (el.availableCards) {
        el.availableCards.classList.remove('pool-highlight-mode-pop','pool-highlight-mode-brightness');
        el.availableCards.classList.add(`pool-highlight-mode-${mode}`);
        el.availableCards.querySelectorAll('.draft-card').forEach(c => {
          const other = c.dataset.cardName;
          if (!other) return;
          c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self','pool-card-hovered');
          if (other === name) {
            // keep hovered draft card bright even if no partners present
            c.classList.add('pool-card-hovered');
            if (anyPartnerPresent) {
              if (hoveredSign > 0) c.classList.add('pool-card-highlight-positive');
              else if (hoveredSign < 0) c.classList.add('pool-card-highlight-negative');
        ...
        if (mode === 'brightness') el.availableCards.classList.add('pool-dimmed');
      }
      // apply to player-picked cards too (only those present)
      if (el.playerBoards) {
        el.playerBoards.classList.remove('pool-highlight-mode-pop','pool-highlight-mode-brightness');
        el.playerBoards.classList.add(`pool-highlight-mode-${mode}`);
        el.playerBoards.querySelectorAll('.player-pick').forEach(c => {
          const other = c.dataset.cardName;
          if (!other) return;
          c.classList.remove('pool-card-highlight-positive','pool-card-highlight-negative','pool-card-highlight-self','pool-card-hovered');
          if (other === name) {
            // keep hovered player-picked card bright even if no partners present
            c.classList.add('pool-card-hovered');
            if (anyPartnerPresent) {
              if (hoveredSign > 0) c.classList.add('pool-card-highlight-positive');
              else if (hoveredSign < 0) c.classList.add('pool-card-highlight-negative');
        ...
        if (mode === 'brightness') el.playerBoards.classList.add('pool-dimmed');
      }
    } else if (source === 'draft') {
      // apply to available draft cards only
      if (el.availableCards) {
        el.availableCards.classList.remove('pool-highlight-mode-pop','pool-highlight-mode-brightness');
        el.availableCards.classList.add(`pool-highlight-mode-${mode}`);
        el.availableCards.querySelectorAll('.draft-card').forEach(c => {
      ...
      // highlight player-picked cards when hovering draft-area cards
      if (el.playerBoards) {
        el.playerBoards.classList.remove('pool-highlight-mode-pop','pool-highlight-mode-brightness');
        el.playerBoards.classList.add(`pool-highlight-mode-${mode}`);
        el.playerBoards.querySelectorAll('.player-pick').forEach(c => {
  ...
  for (const entry of entries) {
    ...
    for (const synergy of SYNERGIES) {
      ...
      if (type === "lore") {
        ...
        // Also detect lore-lore pairs where the CSV/legacy list uses the "leader" field to hold a lore name
        if (entry.name === synergy.leader) {
          // Only treat as lore-lore if the other side exists as a lore in our card list
          const otherIsLore = allLore.some(x => x.name === synergy.lore);
          if (otherIsLore) { hasSynergy = true; break; }
        }
      }
    }
    // For the select-draft pool we intentionally do not show the synergy star
    // (keep synergy info in tooltips/other areas but remove the visual badge here).
    ...
    // mark the DOM element with the card name for easy lookup
    btn.dataset.cardName = entry.name;

    // Use global highlight handlers so available cards and pools behave consistently
    btn.dataset.cardName = entry.name;
    btn.addEventListener('mouseenter', () => applyHighlightsForCard(entry.name, type, 'pool'));
    btn.addEventListener('mouseleave', () => clearPoolHighlights());
    btn.addEventListener('focus', () => applyHighlightsForCard(entry.name, type, 'pool'));
    btn.addEventListener('blur', () => clearPoolHighlights());
    
    // Add synergy info to title (collect numeric bonuses, sort high->low)
    let synergyEntries = [];
    ...
    for (const synergy of SYNERGIES) {
      ...
      if (type === "leader" && entry.name === synergy.leader) {
        ...
        if (entry.name === synergy.lore) {
          ...
          if (bonus > 0) hasPositive = true; else hasNegative = true;
        } else if (entry.name === synergy.leader) {
    ...
    if (synergyEntries.length > 0) {
      synergyEntries.sort((a, b) => b.bonus - a.bonus);
      const synergyInfo = synergyEntries.map(e => e.text);
      let label = 'Bonuses';
...
function getBestPick(playerIdx) {
  if (draft.bestPickAlgo === 'simulation') {
    return getBestPickSim(playerIdx);
  }
  ...
  for (const card of cards) {
    // Immediate unboosted value (used for final-pick decision)
    const immediateNoGuarantee = card.pickType === 'leader'
      ? effectiveLeaderValueNoGuarantee(card)
      : effectiveLoreValueNoGuarantee(card);
    ...
    if (card.pickType === "leader") {
      ...
      if (replacementIdx < sortedLeadersNo.length) {
        ...
        if (matchingLores.length > picksBeforeMyNextLore) {
          ...
          const replacementLoreVal = replacementLore ? effectiveLoreValueNoGuarantee(replacementLore) : 0;
            // Model opponents as taking the highest-value matching lores first
            // and compute the expected partner value as the average of the
            // matching lores that would remain after `picks` are removed.
            ...
            let expectedPartnerVal = 0;
            if (matchingSortedDesc.length > picks) {
              const remaining = matchingSortedDesc.slice(picks);
              const remVals = remaining.map(l => {
                const s = SYNERGIES.find(x => x.leader === card.name && x.lore === l.name);
                const bonus = typeof s?.bonus === 'function' ? s.bonus(draft.numPlayers) : (s?.bonus || 0);
                return getWeightedScore(l, "lore") + bonus;
              });
              expectedPartnerVal = remVals.reduce((a,b) => a+b, 0) / remVals.length;
            } else {
              // No matching partners remain; expected partner value falls back to replacement
              expectedPartnerVal = replacementLoreVal;
            }
            guaranteedBonus = Math.max(0, expectedPartnerVal - replacementLoreVal);
        }
      ...
      if (replacementIdx < sortedLoreNo.length) {
        ...
        if (matchingLeadersEarly.length > picksBeforeMyNextLeader) {
          ...
          // Model opponents as taking highest-value matching leaders first and
          // compute expected leader partner as the average of remaining matching
          // leaders after `picksL` are removed.
          ...
          let expectedLeaderVal = 0;
          if (matchingLeadersDesc.length > picksL) {
            const remaining = matchingLeadersDesc.slice(picksL);
            const remVals = remaining.map(l => {
              const s = SYNERGIES.find(x => x.lore === card.name && x.leader === l.name);
              const bonus = typeof s?.bonus === 'function' ? s.bonus(draft.numPlayers) : (s?.bonus || 0);
              return getWeightedScore(l, "leader") + bonus;
            });
            expectedLeaderVal = remVals.reduce((a,b) => a+b, 0) / remVals.length;
          } else {
            expectedLeaderVal = replacementLeaderVal;
          }
          guaranteedBonus = Math.max(0, expectedLeaderVal - replacementLeaderVal);
        }
        opportunityCost = oc_noBoost + guaranteedBonus;
      } else {
        opportunityCost = cardScore;
    ...
    // If this is our final pick before we stop picking, prefer immediate value
    if (picksBeforeMyNextTurn === 0) {
      opportunityCost = immediateNoGuarantee;
    }
...
function renderPlayerBoards() {
  ...
  for (let i = 0; i < draft.numPlayers; i++) {
    ...
    if (player.leader) {
      ...
      // Check for synergy star and build tooltip lines
      let hasSynergy = false;
      const synergyEntries = [];
      let hasPositive = false;
      let hasNegative = false;
      for (const synergy of SYNERGIES) {
      ...
      if (synergyEntries.length > 0) {
        ...
        titleAttr = ` title="${escaped}"`;
      }
      
      picksHTML += `<div class="player-pick" data-card-name="${player.leader.name.replace(/"/g, '&quot;')}"${titleAttr}>
        ${imgTag}
    ...
    for (let j = 0; j < loreCount; j++) {
        ...
        // Check for synergy star and build tooltip
        let hasSynergy = false;
        const synergyEntriesLore = [];
        let hasPositiveL = false;
        let hasNegativeL = false;
        if (player.leader) {
          for (const synergy of SYNERGIES) {
        ...
        // lore-lore: check other owned lore for pairings (both CSV table and legacy entries)
        for (let k = 0; k < player.lore.length; k++) {
          if (k === j) continue;
          const other = player.lore[k];
          // Table-based lookups
          if (SYNERGY_TABLE) {
            const rowA = [normalizeText(other.name)];
            const colA = [normalizeText(player.lore[j].name)];
            ...
            const rowB = [normalizeText(player.lore[j].name)];
            const colB = [normalizeText(other.name)];
          ...
          // Legacy SYNERGIES entries
          for (const synergy of SYNERGIES) {
            if (player.lore[j].name === synergy.lore && other.name === synergy.leader) {
              const bonus = typeof synergy.bonus === 'function' ? synergy.bonus(draft.numPlayers) : synergy.bonus;
              synergyEntriesLore.push({ text: `${bonus > 0 ? '+' : ''}${Number(bonus).toFixed(1)} pts with ${other.name}`, bonus: Number(bonus) });
              if (bonus > 0) hasPositiveL = true; else hasNegativeL = true;
              hasSynergy = true;
            }
            if (player.lore[j].name === synergy.leader && other.name === synergy.lore) {
              const bonus = typeof synergy.bonus === 'function' ? synergy.bonus(draft.numPlayers) : synergy.bonus;
              synergyEntriesLore.push({ text: `${bonus > 0 ? '+' : ''}${Number(bonus).toFixed(1)} pts with ${other.name}`, bonus: Number(bonus) });
              if (bonus > 0) hasPositiveL = true; else hasNegativeL = true;
              hasSynergy = true;
        ...
        let titleAttrLore = "";
        if (synergyEntriesLore.length > 0) {
          synergyEntriesLore.sort((a,b) => b.bonus - a.bonus);
          const synergyInfoLore = synergyEntriesLore.map(e => e.text);
          let labelL = 'Bonuses';
          if (hasPositiveL && !hasNegativeL) labelL = 'Synergies';
          else if (!hasPositiveL && hasNegativeL) labelL = 'Anti-synergies';
          else if (hasPositiveL && hasNegativeL) labelL = 'Synergies/Anti-synergies';
          const linesL = [labelL, ...synergyInfoLore];
          const escapedL = linesL.join('\n').replace(/"/g, '&quot;');
          titleAttrLore = ` title="${escapedL}"`;
        }
        
        picksHTML += `<div class="player-pick" data-card-name="${player.lore[j].name.replace(/"/g, '&quot;')}"${titleAttrLore}>
          ${imgTag}
          <div class="player-pick-details">
            <div class="pick-header">
    ...
    // attach hover handlers for player picks (link to draft-area highlights)
    try {
      board.querySelectorAll('.player-pick').forEach(pp => {
        const cname = pp.dataset.cardName;
        if (!cname) return;
        pp.addEventListener('mouseenter', () => applyHighlightsForCard(cname, pp.querySelector('.pick-type')?.textContent.trim().toLowerCase() === 'lead' ? 'leader' : 'lore', 'draft'));
        pp.addEventListener('mouseleave', () => clearDraftHighlights());
        pp.addEventListener('focus', () => applyHighlightsForCard(cname, pp.querySelector('.pick-type')?.textContent.trim().toLowerCase() === 'lead' ? 'leader' : 'lore', 'draft'));
        pp.addEventListener('blur', () => clearDraftHighlights());
      });
    } catch (e) {}
  }
}
...
function renderAvailableCards() {
  ...
  function renderCardGroup(cards, label) {
    ...
    for (const card of cards) {
      ...
      // Check for synergies — show only if the partner card is in the draft
      ...
      for (const synergy of SYNERGIES) {
        ...
        if (card.pickType === "leader" && card.name === synergy.leader) {
          ...
          if (partnerInDraft) { showsSynergy = true; break; }
        } else if (card.pickType === "lore") {
          // card matches right-side (synergy.lore)
          if (card.name === synergy.lore) {
            const partnerInDraft = draft.availableLeaders.some(l => l.name === synergy.leader) ||
                                    draft.players.some(p => p.leader && p.leader.name === synergy.leader) ||
                                    // also consider lore-lore where synergy.leader is actually a lore
                                    draft.availableLore.some(l => l.name === synergy.leader) ||
                                    draft.players.some(p => p.lore.some(l => l.name === synergy.leader));
            if (partnerInDraft) { showsSynergy = true; break; }
          }
          // card matches left-side (synergy.leader) — indicates a lore-lore pair where partner is synergy.lore
          if (card.name === synergy.leader) {
            const partnerInDraft = draft.availableLore.some(l => l.name === synergy.lore) ||
                                    draft.players.some(p => p.lore.some(l => l.name === synergy.lore));
            if (partnerInDraft) { showsSynergy = true; break; }
      ...
      if (showsSynergy) {
        ...
        for (const synergy of SYNERGIES) {
          ...
          if (card.pickType === "leader" && card.name === synergy.leader) {
            ...
            // card matches right-side (synergy.lore) — partner may be a leader or another lore
            if (card.name === synergy.lore) {
              const partnerInDraft = draft.availableLeaders.some(l => l.name === synergy.leader) ||
                                      draft.players.some(p => p.leader && p.leader.name === synergy.leader) ||
                                      draft.availableLore.some(l => l.name === synergy.leader) ||
            ...
            // card matches left-side (synergy.leader) — treat as lore-lore where partner is synergy.lore
            if (card.name === synergy.leader) {
              const partnerInDraft = draft.availableLore.some(l => l.name === synergy.lore) ||
                                      draft.players.some(p => p.lore.some(l => l.name === synergy.lore));
              if (!partnerInDraft) continue;
      ...
      // compute heuristic HTML for Deep (simulation) mode
      let heuristicHTML = '';
      try {
        if (draft.bestPickAlgo === 'simulation' && draft.lastSimScores) {
          const k = card.name + '|' + card.pickType;
          const hv = draft.lastSimScores[k];
          if (hv !== undefined && hv !== null && !Number.isNaN(Number(hv))) {
            heuristicHTML = `<div class="draft-card-heuristic">μ: ${Number(hv).toFixed(1)} pts</div>`;
          }
        }
      } catch (e) { heuristicHTML = ''; }

      // tag for lookup and hover highlighting
      ...
      div.addEventListener('mouseenter', () => applyHighlightsForCard(card.name, card.pickType));
      div.addEventListener('mouseleave', () => clearAllHighlights());
      div.addEventListener('focus', () => applyHighlightsForCard(card.name, card.pickType));
      div.addEventListener('blur', () => clearAllHighlights());
...
async function init() {
  ...
  try {
    setStatus("Loading cards, tier data & synergies…");
    const [cards, tierRows, rankedRows, synergies] = await Promise.all([
      loadCards(),
      loadTierList(),
      loadRankedSheet(),
      loadSynergySheet(),
    ...
    // Save loaded synergy table (may be null)
    if (synergies) {
      SYNERGY_TABLE = synergies.table;
      // Append CSV-derived entries into legacy SYNERGIES so existing code paths that
      // iterate SYNERGIES will see the sheet-provided pairs.
      if (Array.isArray(synergies.entries)) {
        for (const e of synergies.entries) {
          // Avoid duplicating identical entries: check existing by leader+lore
          const exists = SYNERGIES.some(s => s.leader === e.leader && s.lore === e.lore);
          if (!exists) SYNERGIES.push({ leader: e.leader, lore: e.lore, bonus: e.bonus });
        }
      }
      setStatus("Synergy CSV loaded — using sheet values.");
      setTimeout(() => setStatus(""), 2500);
    } else {
      console.info("No synergy CSV loaded; falling back to built-in SYNERGIES.");
      setStatus("No synergy CSV loaded; using built-in synergies.");
      setTimeout(() => setStatus(""), 3500);
    }

    // Mobile tooltip support: show element `title` content on touch devices
    (function setupMobileTooltip() {
      let tooltipEl = document.createElement('div');
      tooltipEl.id = 'mobileTooltip';
      tooltipEl.className = 'mobile-tooltip hidden';
      document.body.appendChild(tooltipEl);

      let touchTimer = null;
      let currentTarget = null;

      function showTooltipFor(target, x, y) {
        ...
        let left = x - Math.min(40, x - 8);
        if (left + ttRect.width > window.innerWidth - 8) left = window.innerWidth - ttRect.width - 8;
        if (left < 8) left = 8;
        let top = (y + 16 + ttRect.height > window.innerHeight) ? (y - 8 - ttRect.height) : (y + 16);
        if (top < 8) top = 8;
      ...
      document.addEventListener('touchstart', (ev) => {
        ...
        const target = (ev.target).closest ? ev.target.closest('[title]') : null;
        // If there is an existing persistent tooltip and the touch is outside it, hide it
        if (tooltipEl.classList.contains('visible') && !ev.target.closest && !ev.target.closest('#mobileTooltip') && target == null) {
          hideTooltip();
          return;
        }
        if (!target) return;
        currentTarget = target;
        // Delay so short taps still trigger click; long-press shows tooltip
        touchTimer = setTimeout(() => {
          showTooltipFor(target, t.clientX, t.clientY);
          persistent = true; // make tooltip stay after touchend
        }, 300);
      }, { passive: true });

      document.addEventListener('touchend', (ev) => {
        // If tooltip was made persistent, don't hide on touchend — wait for explicit tap elsewhere
        if (touchTimer) { clearTimeout(touchTimer); touchTimer = null; }
        if (persistent) return;
        hideTooltip();
      });
      document.addEventListener('touchcancel', () => { if (!persistent) hideTooltip(); });

      // Hide tooltip when user taps/clicks anywhere outside the tooltip
      document.addEventListener('click', (ev) => {
        const inside = ev.target.closest && (ev.target.closest('#mobileTooltip') || ev.target.closest('[title]'));
        if (!inside) hideTooltip();
      });

      window.addEventListener('scroll', () => { if (!persistent) hideTooltip(); }, { passive: true });
      window.addEventListener('orientationchange', () => { if (!persistent) hideTooltip(); });
    })();
